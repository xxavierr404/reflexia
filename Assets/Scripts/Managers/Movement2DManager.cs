...
public class Movement2DManager : MonoBehaviour
{
    ...
    private static RaycastHit mirrorTeleportCollision; //Информация о последней коллизии "ног" игрока с отражением в зеркале
    ...
    public static void Movement2D(Vector3 move)
    {
        ...
        if (!CheckHorizontalCollision(move) && !CheckHorizontalCollisionOnCenter()) //Движение при отсутствии препятствий на горизонтали
        {
            Vector3 velocity = player.rigidBody.velocity;
            player.rigidBody.AddForce(move * player.speed - new Vector3(velocity.x, 0, velocity.z), ForceMode.VelocityChange);
        }
        ...
        {
            player.rigidBody.AddForce(player.transform.up - new Vector3(0, player.rigidBody.velocity.y, 0), ForceMode.VelocityChange);
    ...
    private static bool CheckVerticalCollisionOnFeet()
    {
        ...
        Ray mirrorCollisionRayVertical = new Ray(position, (transform.position - transform.up) - position);
    ...
    private static bool CheckVerticalCollisionOnHead()
    {
        var transform = player.transform;
        var position = mirrorCam.position;
        Ray mirrorCollisionRayVertical = new Ray(position, (transform.position + transform.up*0.8f) - position);
    ...
    {
        var position = mirrorCam.position;
        Ray mirrorCollisionRayHorizontal = new Ray(position, (player.transform.position + new Vector3(0.5f * moveSign, Mathf.Clamp(player.rigidBody.velocity.y * (-2.0f), -1.0f, 1.0f), 0)) - position);
    ...
    private static bool CheckHorizontalCollision(Vector3 move)
    {
        var transform = player.transform;
        var position = mirrorCam.position;
        Ray mirrorCollisionRayHorizontal = new Ray(position, (transform.position - transform.up * 0.7f + move * 0.8f) - position);
        return Physics.Raycast(mirrorCollisionRayHorizontal, Mathf.Infinity, layerMask);
    }
}